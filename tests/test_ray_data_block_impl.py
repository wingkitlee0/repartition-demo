...
def test_split_block_single_block(pyarrow_block_0):
    keys_and_blocks = list(split_block(pyarrow_block_0, "x"))
...
def test_split_block_2_blocks(pyarrow_block_1):
    keys_and_blocks = list(split_block(pyarrow_block_1, "x"))
...
def test_split_block_2_blocks_2_keys(pyarrow_block_1):
    keys_and_blocks = list(split_block(pyarrow_block_1, ["x", "y"]))

    assert len(keys_and_blocks) == 4
    assert [k for k, _ in keys_and_blocks] == [(1, 2), (1, 3), (2, 4), (2, 5)]


def test_split_block_empty_block(empty_block):
    keys_and_blocks = list(split_block(empty_block, "x"))
    assert len(keys_and_blocks) == 0


def test_split_block_non_varied_block(non_varied_block):
    keys_and_blocks = list(split_block(non_varied_block, "x"))
    assert len(keys_and_blocks) == 1
    assert keys_and_blocks[0][0] == 1


def test_split_block_non_numeric_block(non_numeric_block):
    keys_and_blocks = list(split_block(non_numeric_block, "x"))
    assert len(keys_and_blocks) == 2
...
def test_split_block_single_row_block(single_row_block):
    keys_and_blocks = list(split_block(single_row_block, "x"))
    assert len(keys_and_blocks) == 1
    assert keys_and_blocks[0][0] == 1


def test_split_block_multiple_keys(non_numeric_block):
    keys_and_blocks = list(split_block(non_numeric_block, ["x", "y"]))
    assert len(keys_and_blocks) == 2
